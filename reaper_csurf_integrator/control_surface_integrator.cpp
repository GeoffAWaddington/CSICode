...
static void ProcessMidiWidget(int &lineNumber, ifstream &surfaceTemplateFile, vector<string> tokens, Midi_ControlSurface* surface)
{
    ...
    for(int i = 0; i < tokenLines.size(); i++)
    {
        ...
        else if(widgetClass == "XTouchCompactEncoder" && size == 4)
            new XTouchCompact7BitRotaryToEncoder_Midi_CSIMessageGenerator(surface, widget, new MIDI_event_ex_t(strToHex(tokenLines[i][1]), strToHex(tokenLines[i][2]), strToHex(tokenLines[i][3])));
